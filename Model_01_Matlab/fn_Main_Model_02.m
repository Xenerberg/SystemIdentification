...
%     Mu_noise(a+1:b,:) = quatnormalize(Mu(a+1:b,:) + normrnd(0,sqrt(1e-4),b-a,4));
%     Mu_noise(b+1:end,:) = quatnormalize(Mu(b+1:end,:) + normrnd(0,sqrt(1e-4),length(X_a(b+1:end,1)),4));
...
%         end
        ...
        eigMin(iCount) = min(real(eig(A-K*Hk)));
    end
    toc;
    save('Data\trPi_M2Noise.mat','tr_M');
