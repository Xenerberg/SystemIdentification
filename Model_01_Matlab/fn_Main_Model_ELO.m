...
%data (Extended Luenberger Observer/Quaternion algebra)
%Author: Hrishik Mishra

%Function: fn_Main()
%Inputs: none
%Outputs:none
%Functionality: Initialize the system/Execute the Observer/Display results
%Author: Hrishik Mishra
...
%         set(h1_stem, 'BaseValue',mean(quatnorm(dcm2quat(Signal_Transf.Data))));
    ...
    a = 200;
    b = 300;
    Mu_noise(1:a,:) = quatnormalize(Mu(1:a,:) + 1e-2*randn(a,4));
    Mu_noise(a+1:b,:) = quatnormalize(Mu(a+1:b,:) + 1e-2*randn(b-a,4));
    Mu_noise(b+1:end,:) = quatnormalize(Mu(b+1:end,:) + 1e-2*randn(length(X_a(b+1:end,1)),4));
    ...
    r_c(:,1:a) = r_c(:,1:a) + 3e-2*randn(a,3)';
    r_c(:,a+1:b) = r_c(:,a+1:b) + 3e-2*randn(b-a,3)';
    r_c(:,b+1:end) = r_c(:,b+1:end) + 3e-2*randn(length(X_a(b+1:end,1)),3)';
    ...
    %%
    %X_a_0 = X_a_0_sim.X_a_0_Model_02;
...
%     P_post(10:12,10:12) = eye(3,3);    
    ...
    residual = zeros(length(v_time)-1,1);
    signal = zeros(7,1);
    ...
    for iCount = 2:length(v_time)        
        ...
        hk = fn_Create_h(X_a_Estimated(1:4,iCount-1),X_a_Estimated(:,iCount-1),rho);
        residual(iCount,:) = norm(zk - hk);
        %Compute Jacobians
        A = fn_Create_A(X_a_Estimated(:,iCount-1),n,p);
        H = fn_Create_H(q_nominal,rho);
        poles_System = -[210,214,212,400+100i,400-100i,406,200,220,210,0.0012,0.0011,0.007]/250;
        %poles_System = (91:102)/100;
        if iCount > length(v_time)
           K = zeros(12,6); 
           %options = odeset('RelTol',1e-2,'AbsTol',1e-0*ones(13,1));
           %[~,X] = ode45(@(t,y)fn_StateSpace(t,X_a_Estimated(:,iCount-1)),[v_time(iCount-1),v_time(iCount)],X_a_Estimated(:,iCount-1),options);                
           %Euler's forward method          
           [X] = fn_StateSpace(0,X_a_Estimated(:,iCount-1));
           X_a_Estimated(:,iCount) = X_a_Estimated(:,iCount-1) + t_delta*X;
           %X_a_Estimated(:,iCount) = X(end,:)';
           X_a_Estimated(1:4,iCount) = X_a_Estimated(1:4,iCount)/norm(X_a_Estimated(1:4,iCount));
           
        else
            K = place(A',H',poles_System)';
            %% Run the dynamic model to get Observed states        
            X = [zeros(3,1);X_a_Estimated(5:13,iCount-1)];
            %[~,X] = ode45(@(t,y)fn_StateSpaceELO(t,X_a_Estimated(:,iCount-1),K,zk,hk),[v_time(iCount-1),v_time(iCount)],X_a_Estimated(:,iCount-1),options);                
            %options = odeset('RelTol',1e-7,'AbsTol',1e-9*ones(12,1));
            %[~,X] = ode45(@(t,y)fn_StateSpaceELO2(t,X,K,zk,hk),[v_time(iCount-1),v_time(iCount)],X,options);                
            [dX] = fn_StateSpaceELO2(0,X,K,zk,hk);
            X = X + t_delta*dX;
            del_q_v = X(1:3);
            del_q_0 = sqrt(1-norm(del_q_v)^2);
            del_q = [del_q_v;del_q_0];
            q = fn_CrossTensor(del_q,0)*q_nominal;
            X_a_Estimated(:,iCount) = [q;X(4:12)];
            %X_a_Estimated(:,iCount) = X(end,:)';          
            %X_a_Estimated(1:4,iCount) = quatnormalize(X_a_Estimated(1:4,iCount)');
        
        end
%         correction_factor = K*(zk-hk);         
%         del_q_v = correction_factor(1:3);    
%         del_q_0 = sqrt(1-norm(del_q_v)^2);
%         if norm(del_q_v) >= 1
%             pause; 
%         end
        ...
        [eig_vectorObs,eig_Observer] = eig(A - K*H);
    ...
    for iCount = 1:size(X_a_Estimated,2)
...
%Functionality: Implements the State-space model of the dynamic system
%Author: Hrishik Mishra
function[dy] = fn_StateSpaceELO(~,X_a,K,zk,hk)
    global n;
    Init_Model_02_Matlab;
    v_n = [0;0; n];
    q = X_a(1:4);
    ...
    %q_corr = fn_CrossTensor(del_q,0)*q;
    J_k = [1 0 0;0 (1-p(2))/(1+p(1)) 0;0 0 (1+p(3))/(1-p(1))];
    q_omega_rel = [omega;0];%Check this for the Attitude Control of the Chaser
    psi = [p(1)*omega(2)*omega(3); p(2)*omega(1)*omega(3);p(3)*omega(1)*omega(2)];
    %q_dot = 0.5*fn_CrossTensor([del_q_v;del_q_0],0)*fn_CrossTensor(q_omega_rel,0)*q;
    q_dot = 0.5*fn_CrossTensor(q,1)*([omega + del_q_v;0]);
    tau = [tau_1;tau_2;tau_3]*rand;
    e_force = [e_force_x;e_force_y;e_force_z]*rand;
    omega_dot = psi + J_k*correction_factor(4:6); %for static conditions
    r_ddot = zeros(3,1);%fn_Compute_r_ddot(n,r,e_force,r_dot);
    
    temp = [r_dot;r_ddot] + correction_factor(7:12);
    %r_dot = zeros(3,1);
    %r_ddot = zeros(3,1);
    
    dy = [q_dot;omega_dot;temp];
end
...
%Functionality: Implements the State-space model of the dynamic system
%Author: Hrishik Mishra
function[dy] = fn_StateSpaceELO2(~,X,K,zk,hk)
    global n;
    Init_Model_02_Matlab;
    v_n = [0;0; n];
    a_g = X(1:3);
...
%     end
    ...
    a_g_dot = -fn_VectorToSkewSymmetricTensor(omega)*a_g + del_q_v;%+0.5*correction_factor(4:6);
    J_k = [1 0 0;0 (1-p(2))/(1+p(1)) 0;0 0 (1+p(3))/(1-p(1))];
    q_omega_rel = [omega;0];%Check this for the Attitude Control of the Chaser
    psi = [p(1)*omega(2)*omega(3); p(2)*omega(1)*omega(3);p(3)*omega(1)*omega(2)];
    %q_dot = 0.5*fn_CrossTensor(q_omega_rel,0)*q;
    ...
    tau = [tau_1;tau_2;tau_3]*rand;
    e_force = [e_force_x;e_force_y;e_force_z]*rand;
    omega_dot = psi + correction_factor(4:6);%J_k*correction_factor(4:6); %for static conditions
    r_ddot = fn_Compute_r_ddot(n,r,e_force,r_dot);
