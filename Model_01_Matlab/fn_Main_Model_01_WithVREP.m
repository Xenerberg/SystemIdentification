...
%     
    ...
    omegaIRL_real = zeros(3,length(Time));
    
    X_a_0_sim = load('./Model_01_Matlab/X_a_0_est_1');
    ...
    if (clientID > -1)
        ...
        h_clientSatEst = Objects_Scene(48);
        h_clientSatReal = Objects_Scene(49);
        ...
        for iCount = 2:length(v_time)
            ...
            %Find angular momentum in body frame
            L(:,iCount) = fn_CreateRotationMatrix(X_a_Estimated(1:4,iCount))*(InM*X_a_Estimated(5:7,iCount));
        ...
        figure;
        subplot(2,1,1);
        plot(dy_time,omegaIRL(1:3,:));hold all;
        plot(dy_time,omegaIRL_real(1:3,:),'LineStyle','-.');
        subplot(2,1,2);
