...
%         set(h1_stem, 'BaseValue',mean(quatnorm(dcm2quat(Signal_Transf.Data))));
    ...
    Mu_noise(1:a,:) = quatnormalize(Mu(1:a,:) + normrnd(0,sqrt(1e-4),[a,4]));
    Mu_noise(a+1:b,:) = quatnormalize(Mu(a+1:b,:) + normrnd(0,sqrt(5e-2),[b-a,4]));
    Mu_noise(b+1:end,:) = quatnormalize(Mu(b+1:end,:) + normrnd(0,sqrt(5e-2),[length(X_a(b+1:end,1)),4]));
    Mu_noise = [Mu_noise(:,1:3), Mu_noise(:,4)];
    sigma_z = zeros(length(Mu));
    sigma_z(1:a,1) = 1e-4;
    sigma_z(a+1:b,:) = 5e-2;
    sigma_z(b+1:end,:) = 5e-2;
    ...
    r_c_true = r_c;
    r_c(:,1:a) = r_c(:,1:a) + normrnd(0,sqrt(1e-4),[a,3])';
    r_c(:,a+1:b) = r_c(:,a+1:b) + normrnd(0,sqrt(5e-2),[b-a,3])';
    r_c(:,b+1:end) = r_c(:,b+1:end) + normrnd(0,sqrt(5e-2),[length(X_a(b+1:end,1)),3])';
    ...
    signal = zeros(7,1);
    test = zeros(6,length(v_time));
    Sk = diag(beta./alpha);%Initial meas covar
    SkDiag(:,1) = beta./alpha;
    %%
    ...
    for iCount = 2:length(v_time)
        ...
        for vbCounter = 1:3
            ...
            Sk = diag(beta./alpha);
            SkDiag(:,iCount) = beta./alpha;   
            
            K = fn_ComputeKalmanGain(P_pre,Hk,Sk);                    
            residuals(iCount-1,:) = zk - post_h;
            X_pre_estimate(:,iCount) = X_pre(:,end) + K*(zk - post_h);            
            ...
            post_h = fn_Create_h(q_est,X_pre_estimate(:,iCount), rho);
            post_residual = zk - post_h;
            Hk = fn_Create_H(q_est,rho);
            matA = Hk*P_post*Hk';
            beta = beta + 0.5*post_residual.^2 + 0.5*diag(matA);
            if norm(X_pre_estimate(:,iCount) - xprev) < 1e-3
               fprintf('counter:%d\n',vbCounter);
                break;                
            end
            xprev = X_pre_estimate(:,iCount);
        end
        ...
        Mh_PostDist(iCount-1) = sqrt(post_residual'*inv(Hk_post*P_post*Hk_post' + Sk)*post_residual);
        ...
        Sk_true = sigma_z(iCount)*eye(6,6);
    ...
    toc;
    save('Data\trPi_M2NoiseVB.mat','tr_M');
    ...
    for iCount = 1:length(X_a_Estimated)
       ...
       err_r_c(iCount) = norm(r_c_est(1:3,iCount) - r_c_true(1:3,iCount));
...
%     ylabel('p');
    ...
    plot(dy_time, mean(sq_error(200:end))*ones(size(dy_time)),'LineStyle','-.');
    ylim([0,2e-1]);
    ...
    figure;
    plot(dy_time,sigma_z,'LineStyle','-.','LineWidth',2);hold all;
    plot(dy_time, SkDiag(1,:),'LineStyle','-.','LineWidth',1);
    plot(dy_time, SkDiag(2,:),'LineStyle','-.','LineWidth',1);
    plot(dy_time, SkDiag(3,:),'LineStyle','-.','LineWidth',1);
    plot(dy_time, SkDiag(4,:),'LineStyle','-.','LineWidth',1);
    plot(dy_time, SkDiag(5,:),'LineStyle','-.','LineWidth',1);
    plot(dy_time, SkDiag(6,:),'LineStyle','-.','LineWidth',1);
    ylim([0,1e-1]);
    ylabel('$R_{ii}$','interpreter','latex','FontSize', 15);
